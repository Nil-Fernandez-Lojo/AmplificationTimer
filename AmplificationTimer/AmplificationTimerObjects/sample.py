...
class Sample():
	...
	def preprocess_data(self,threshold_amplification_no_WGD = 5):
		...
		self.subclonal_structure = pd.read_csv(self.config["path_folder_subclonal_structure"]/ (self.name+self.config['suffix_subclonal_structure']), sep='\t')
	...
	def load_preprocessed_data(self,path_preprocessed_data):
		with open(self.config['path_folder_preprocessed_data']/ (self.name+'.json'), 'r') as fp:
			data = json.load(fp)
