...
def get_timing_oncogene(oncogene, cancer_type, amplifications, summary_traces, summary_table):
    ...
    for i, a in enumerate(amplifications_filtered):
        if i == 0 or (
                a['clinical_data']['samplename'] != amplifications_filtered[i - 1]['clinical_data']['samplename']):
            amplification_id = 0
        else:
            amplification_id += 1
        for gene in a['oncogenes']:
...
def get_figure_3(oncogene, cancer_type, amplifications, summary_traces_4, summary_traces_1, summary_table,
...
if subclonality_modelled:
    ...
    path_summary_traces_1 = Path("inferred_times/traces") / "subclonality_not_modelled" / (
                "Model" + str(1)) / "summary.pkl"
    path_summary_traces_4 = Path("inferred_times/traces") / "subclonality_not_modelled" / (
                "Model" + str(4)) / "summary.pkl"
